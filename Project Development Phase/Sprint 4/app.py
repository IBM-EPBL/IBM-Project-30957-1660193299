...
app.config['uid'] = 'cyh92419'
app.config['pwd'] = 'gDXbkJ1sdM82Hcct'
app.config['security'] = 'SSL'
try:
    mysql = DB2(app)

    conn_str='database=bludb;hostname=8e359033-a1c9-4643-82ef-8ac06f5107eb.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;port=30120;protocol=tcpip;\
            uid=cyh92419;pwd=gDXbkJ1sdM82Hcct;security=SSL'
...
def addAccount():
  if request.method == 'POST':
    ...
    insertConfigurations(id, "Expense", "Food & Beverages", float(predictable_monthly_income) * 0.15, today)
    insertConfigurations(id, "Expense", "Groceries", float(predictable_monthly_income) * 0.25, today)
    insertConfigurations(id, "Expense", "Fuel", float(predictable_monthly_income) * 0.15, today)
    insertConfigurations(id, "Expense", "Entertainment", float(predictable_monthly_income) * 0.10, today)
    insertConfigurations(id, "Expense", "Other Expenses", float(predictable_monthly_income) * 0.10, today)
